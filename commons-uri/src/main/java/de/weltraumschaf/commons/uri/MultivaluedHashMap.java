 ...
 * behavior can be customized by overriding the protected {@code #addNull(List) addNull(...)} and
 ...
 * and <i>load factor</i>. The <i>capacity</i>
 * is the number of buckets in the hash table, and the initial capacity is simply the capacity at the time the hash
 * table is created. The <i>load factor</i> is a measure of how full the hash table is allowed to get before its
 * capacity is automatically increased. When the number of entries in the hash table exceeds the product of the load
 * factor and the current capacity, the hash table is
 * <em>rehashed</em> (that is, internal data structures are rebuilt) so that the hash table has approximately twice the
 * number of buckets.
 * </p>
 * <p>
 ...
 * <tt>HashMap</tt> class, including <tt>get</tt> and <tt>put</tt>). The expected number of entries in the map and its
 ...
 * If multiple threads access a hash map concurrently, and at least one of the threads modifies the map structurally, it
 * <i>must</i> be synchronized externally. (A structural modification is any operation that adds or deletes one or more
 * mappings; merely changing the value associated with a key that an instance already contains is not a structural
 * modification.) This is typically accomplished by synchronizing on some object that naturally encapsulates the map.
 * </p>
 * <p>
 ...
 * <tt>remove</tt> method, the iterator will throw a {@code  ConcurrentModificationException}. Thus, in the face of
...
class MultivaluedHashMap<K, V> extends AbstractMultivaluedMap<K, V> implements Serializable {
    ...
    /**
     * Constructs an empty multivalued hash map with the default initial capacity ({@code 16}) and the default load
     * factor ({@code 0.75}).
     */
    MultivaluedHashMap() {
    ...
    /**
     * Constructs a new multivalued hash map with the same mappings as the specified single-valued {@code  Map }.
     *
    ...
    /**
     * Constructs a new multivalued hash map with the same mappings as the specified {@code  MultivaluedMap }. The
     * {@code  List} instances holding the values of each key are created anew instead of being reused.
     *
